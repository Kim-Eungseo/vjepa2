...
from app.vjepa.utils import init_opt, init_vicreg_pooler, init_video_model, load_checkpoint
...
def main(args, resume_preempt=False):
    ...
    vicreg_inv_coeff = cfgs_vicreg.get("inv_coeff", 1.0)
    vicreg_std_coeff = cfgs_vicreg.get("std_coeff", 1.0)
    vicreg_cov_coeff = cfgs_vicreg.get("cov_coeff", 1.0)
    ...
    vicreg_loss_fn = VICRegLoss(
        inv_coeff=vicreg_inv_coeff,
        std_coeff=vicreg_std_coeff,
        cov_coeff=vicreg_cov_coeff,
    )
    logger.info(f"VICReg: inv={vicreg_inv_coeff}, std={vicreg_std_coeff}, cov={vicreg_cov_coeff}")
    ...
    if world_size > 1:
        ...
        vicreg_pooler = DistributedDataParallel(vicreg_pooler, static_graph=True)
    ...
    def save_checkpoint(epoch, path):
        ...
        save_dict = {
            ...
            "vicreg_pooler": vicreg_pooler.state_dict(),
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for itr in range(ipe):
            ...
            def train_step():
                ...
                def vicreg_fn(z1_list, z2_list):
                    """Compute VICReg loss between paired clip representations."""
                    vic_loss_total = 0
                    vic_dict = None
                    n = 0
                    for z1_tokens, z2_tokens in zip(z1_list, z2_list):
                        # z1_tokens, z2_tokens: [B, N, D] (sequence of patch tokens)
                        z1 = vicreg_pooler(z1_tokens).squeeze(1)  # [B, D]
                        z2 = vicreg_pooler(z2_tokens).squeeze(1)  # [B, D]
                        vic_dict = vicreg_loss_fn(z1, z2)
                        vic_loss_total += vic_dict["loss"]
                        n += 1
                    if n > 0:
                        vic_loss_total /= n
                    return vic_loss_total, vic_dict

                # Step 1. Forward
                with torch.amp.autocast("cuda", dtype=dtype, enabled=mixed_precision):
                    z1_list = forward_encoder(clips1)  # list of [B, N, D]
                    z2_list = forward_encoder(clips2)  # list of [B, N, D]
                    loss, vic_dict = vicreg_fn(z1_list, z2_list)
                ...
                return (
                    float(loss),
                    {k: float(v) for k, v in vic_dict.items()},
                    _new_lr,
                    _new_wd,
                )

            result, gpu_etime_ms = gpu_timer(train_step)
            loss, vic_dict, _new_lr, _new_wd = result
            vicreg_inv_meter.update(vic_dict["invariance_loss"])
            vicreg_var_meter.update(vic_dict["var_loss"])
            vicreg_cov_meter.update(vic_dict["cov_loss"])
            ...
            def log_stats():
                csv_logger.log(
                    epoch + 1, itr, loss,
                    vic_dict["invariance_loss"], vic_dict["var_loss"], vic_dict["cov_loss"],
                    _new_lr, _new_wd,
                ...
                if (itr % log_freq == 0) or (itr == ipe - 1) or np.isnan(loss) or np.isinf(loss):
                    logger.info(
                        ...
                        % (
                            ...
                            itr,
                            loss_meter.avg,
                            vicreg_inv_meter.avg,
                            vicreg_var_meter.avg,
                            vicreg_cov_meter.avg,
                            _new_wd,
                            _new_lr,
            ...
            if use_wandb and rank == 0:
                wandb.log(
                    {
                        "loss": loss,
                        "inv_loss": vic_dict["invariance_loss"],
                        "var_loss": vic_dict["var_loss"],
                        "cov_loss": vic_dict["cov_loss"],
                        "lr": _new_lr,
                        "wd": _new_wd,

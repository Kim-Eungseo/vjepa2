...
def replay_worker(args_tuple):
    ...
    (traj_path, output_dir, task_id, anchor_info, anchor_pose,
     episode_indices, json_data) = args_tuple
    ...
    for idx in episode_indices:
        ...
        print(f"  [{pid}] ep{episode['episode_id']:03d}: {n_frames} frames")
...
def main():
    ...
    if args.workers <= 1:
        replay_worker((
            ...
            anchor_info, anchor_pose, all_indices, json_data,
        ))
    else:
        ...
        for i, idx in enumerate(all_indices):
            chunks[i % args.workers].append(idx)
        ...
        for chunk in chunks:
            if chunk:
                jobs.append((
                    args.traj_path, args.output_dir, task_id,
                    anchor_info, anchor_pose, chunk, json_data,
                ))

        with Pool(args.workers) as pool:
            pool.map(replay_worker, jobs)

...
import logging
class SuppressActorWarning(logging.Filter):
    def filter(self, record):
        return "No initial pose set for actor builder" not in record.getMessage()
logging.getLogger("mani_skill ").addFilter(SuppressActorWarning())
...
def get_mask_from_obs(obs, seg_ids):
    # Ensure we use the base_camera which has the correct 512x512 resolution
    # Using list(obs["sensor_data"].keys())[0] can sometimes pick "hand_camera" (256x256)
...
def _setup_env(task_id, env_kwargs_base, dr_type, config, anchor_info):
    ...
    if dr_type == "camera":
        ...
        env_kwargs["human_render_camera_configs"] = {
            "shader_pack": "default",
            "render_camera": {"pose": pose, "width": config.get("_res", 512), "height": config.get("_res", 512)},
        }
    elif dr_type == "background":
        ...
        env_kwargs["human_render_camera_configs"] = {
            "shader_pack": "default",
            "render_camera": {"pose": anchor_pose, "width": config.get("_res", 512), "height": config.get("_res", 512)},
        }
        env_kwargs["sensor_configs"] = {
            "base_camera": {
                "pose": anchor_pose, "width": config.get("_res", 512), "height": config.get("_res", 512), "fov": 1,
            }
        }
    else:  # lighting
        anchor_pose = sapien_utils.look_at(anchor_info["eye"], anchor_info["target"])
...
def _replay_episode(env, h5_file, episode, dr_type, config_id, config,
    ...
    if dr_type == "background":
        ...
        if config.get("floor") is not None:
            ...
            res = config.get("_res", 512)
            floor_bg_image = cv2.resize(img, (res, res))
...
def replay_episode_worker(args_tuple):
    ...
    (traj_path, output_dir, dr_type, ep_idx,
     ...
     json_data, n_samples, base_seed, explicit_config_ids, resolution) = args_tuple
    ...
    for cid in config_ids:
        config = all_configs[dr_type][cid].copy()
        config["_res"] = resolution  # Pass resolution down to env setup
...
def main():
    ...
    parser.add_argument("--resolution", type=int, default=512,
                        help="Resolution for rendering and background textures (default: 512)")
    ...
    for ep_idx in episode_indices:
        jobs.append((
            ...
            json_data, n_samples, args.seed, explicit_ids, args.resolution
        ))
